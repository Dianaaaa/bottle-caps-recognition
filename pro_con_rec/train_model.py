...
def get_descriptors(dataset):
    ...
    folders=['cons/','pros/']    # cons(fan) is 0， pros(zheng) is 1
    ...
    for folder in folders:
        ...
        filepath = './' + dataset + '/' + folder
        filenames=os.listdir(filepath)

        for image_name in filenames:
            print(os.path.join(filepath, image_name))
            img=cv2.imread(os.path.join(filepath, image_name))
...
def cluster_features_train(img_descs, cluster_model):
    ...
    joblib.dump(cluster_model, "knn_model.m")
...
def train_and_test_model(classifier, Xtrain, ytrain, Xtest, ytest, is_neural_net=False):
    ...
    joblib.dump(classifier, "svm_model.m")
...
while NUMBER_OF_CLUSTERS <= 252:
    ...
    NUMBER_OF_CLUSTERS = NUMBER_OF_CLUSTERS + 50
